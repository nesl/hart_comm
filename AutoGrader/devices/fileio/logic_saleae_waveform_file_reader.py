...
period of the waveform is 1 second. The display block (surrounding by "Display start" and
"Display end" lines) specifies the configuration of all the plots, one per line. The configuration
includes the plot label, and which pins are included in this plot. In this example, two plots of
waveform are going to be displayed. The first waveform is labeled as "CTL", which captures the
changes of 0th pin. The second waveform is called "VAL", which combines the 2nd pin (the most
significant) and 1st pin (the least significant).
...
Taking the second waveform as an example, we will display 3 transitions:
  - at 0.00 second, the value is 0
  - at 0.63 second, the value is 1
  - at 0.67 second, the value is 0
  - at 0.73 second, the value is 2
...
Or visually, it looks like the following:
  ...
  1             +-----+     |
                |     |     |
  0 +-----------+     +-----+
    0          0.63  0.67  0.73          1  (second)

"""

class LogicSaleaeWaveformFileReader(WaveformQueryBase):
    ...
    def _parse_raw_content(self, raw_content):
        if len(raw_content) == 0:
            self.error_code = LogicSaleaeWaveformFileReader.ERROR_CODE_EMPTY_FILE
            return
        
        try:
            content = raw_content.decode('ascii')
        except:
            self.error_code = LogicSaleaeWaveformFileReader.ERROR_CODE_NON_ASCII
            return

        if not self._parse_content(content):
            self.error_code = LogicSaleaeWaveformFileReader.ERROR_CODE_FORMAT
            return
    ...
    def _parse_content(self, content):
        try:
            ...
            self.data = [(float(l[0]), int(l[1], 16)) for l in line_terms]
    ...
    def get_error_description(self):
        ...
        if self.error_code == LogicSaleaeWaveformFileReader.ERROR_CODE_EMPTY_FILE:
            return "Empty file"
        elif self.error_code == LogicSaleaeWaveformFileReader.ERROR_CODE_NON_ASCII:
            return "Parsing error: The file includes non-ascii characters"
        elif self.error_code == LogicSaleaeWaveformFileReader.ERROR_CODE_FORMAT:
    ...
    def get_event_series(self, series_idx, start_time_sec=None, end_time_sec=None):
        """
        When start_time_sec and/or end_time_sec is None, it is configured as default value:
        start_time_sec will be 0.0, end_time_sec will be the period length.

        Returns:
          (name, time_series)
            - name: plot name, a string
            - time_series: a list of (time_sec, bus_value). Align with both start_time_ms and
